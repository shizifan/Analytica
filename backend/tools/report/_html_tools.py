"""HTML outline-driven LLM agent tools — Step 6.

Mirrors ``_docx_tools.py``: each tool calls the matching
``HtmlBlockRenderer.emit_*`` given a block_id. The agent's job is to
render an already-planned outline, not to compose one.
"""
...
你是 HTML 报告渲染执行者。报告大纲(outline)已经规划完毕，你的任务是按 outline 的顺序，
...
2. 对每个 Section（按编号 0, 1, 2, ... 顺序）:
   ...
   b. 按用户消息中给出的顺序, 对每个 block 调用对应 emit_* 工具(传入 block_id)
...
def make_html_outline_tools(
    ...
    def begin_document() -> str:
        """Initialise the document (KPI overview block). Call once first."""
    ...
    def end_section(section_index: int) -> str:
        """End a section. Must match a prior begin_section."""
        section = _section(section_index)
    ...
    def emit_chart(block_id: str) -> str:
        """Render a chart block (with inline ECharts initialiser)."""
