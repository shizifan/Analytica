...
# ── KPI Strip (辽港数据期刊) — PR-1 ─────────────────────────────────

@dataclass
class KpiStripItem:
    """A single KPI strip cell (辽港数据期刊 四格 strip).

    Attributes:
        label:  Cell label (e.g. "起点", "高点", "当前", "变化").
        value:  Pre-formatted value string (e.g. "7.9%", "-2.3pp").
        sub:    Optional metadata (e.g. "2026-02", "环比").
        trend:  ``"gain"`` | ``"loss"`` | ``""`` — drives colour.
    """
    label: str
    value: str
    sub: str = ""
...
@dataclass
class KpiStripBlock:
    """KPI strip of exactly 4 cells, generated by the planner from
    trend data.  Attached to ``ChartTablePairBlock.kpi_strip``;
    renderers call ``emit_kpi_strip`` for it.

    Not a top-level ``Block`` — it only lives inside chart_table_pair
    or is emitted inline by the planner.
    """
    ...
    def __post_init__(self):
        if len(self.items) != 4:
            raise ValueError(
                f"KpiStripBlock expects exactly 4 items, got {len(self.items)}"
            )
...
class ChartTablePairBlock:
    ...
    subtitle: str = ""
    source: str = ""
    kpi_strip: KpiStripBlock | None = None
...
def _block_to_dict(b: Block) -> dict[str, Any]:
    ...
    if isinstance(b, ChartTablePairBlock) and b.kpi_strip is not None:
        raw["kpi_strip"] = {
            "items": [
                {"label": it.label, "value": it.value,
                 "sub": it.sub, "trend": it.trend}
                for it in b.kpi_strip.items
            ],
        }
    return raw
...
def _block_from_dict(d: dict[str, Any]) -> Block:
    ...
    if cls is ChartTablePairBlock:
        ks = payload.pop("kpi_strip", None)
        if isinstance(ks, dict):
            payload["kpi_strip"] = KpiStripBlock(
                items=tuple(
                    KpiStripItem(
                        label=it.get("label", ""),
                        value=it.get("value", ""),
