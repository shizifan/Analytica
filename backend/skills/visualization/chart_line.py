...
@register_skill("skill_chart_line", SkillCategory.VISUALIZATION, "折线图生成（ECharts option JSON）",
                input_spec="data_ref + title + time_column + value_columns",
...
class LineChartSkill(BaseSkill):
    ...
    async def execute(self, inp: SkillInput, context: dict[str, Any]) -> SkillOutput:
        ...
        # Resolve DataFrame from context — fallback to context_refs, then full context scan
        df = None
        ...
        return SkillOutput(
            ...
            output_type="chart",
            data=option,
            metadata={"chart_type": "line", "series_count": len(series)},
        )
