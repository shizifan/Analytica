...
3. getTrendChart 的 businessSegment 为【必填】单值参数
9. getThroughputAnalysisByYear 的 regionName 为可选参数：不传即返回全港数据；【严禁传入"全港"字符串】，会导致返回空数据，每次调用只能传一个板块名称（字符串，非列表）；若需查询多个板块，必须拆分为多个独立的 data_fetch 任务，每个任务各自传一个 businessSegment
