...
■ chart_text（2-5任务）：
  - 必须≥1个 tool_api_fetch + ≥1个图表技能（tool_chart_bar / tool_chart_line / tool_chart_waterfall）
...
【报告技能参数规范】
所有报告技能（tool_summary_gen / tool_report_html / tool_report_docx / tool_report_pptx）：
...
class PlanningEngine:
    ...
    def _build_prompt(
        ...
        if complexity == "full_report":
            ...
            report_skills = [
                (fmt, format_skill_map.get(fmt, "tool_report_html"))
