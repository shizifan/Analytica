...
9. 【货类匹配原则】用户未明确指定具体货类（如"集装箱""散杂货""商品车"等）时，必须使用综合吞吐量端点（如 getThroughputAndTargetThroughputTon、getThroughputAnalysisByYear、getThroughputByBusinessType），严禁使用集装箱专用端点（如 getContainerThroughputAnalysisByYear、getThroughputAnalysisContainer、getThroughputAndTargetThroughputTeu）。只有用户明确提到"集装箱""TEU""箱量"等关键词时才可使用集装箱专用端点
10. 【月度区间选端规则】当 time_range 跨越多个月份（如"1-4月""Q1""上半年"），且用户未明确要求"累计/汇总/合计"时，必须选择 T_TREND 类型端点（如 getThroughputAnalysisByYear、getContainerThroughputAnalysisByYear、getPersonalCenterYearCargoThroughputTrend 等），返回按月拆分的趋势数据。仅当用户明确说"累计""汇总""总计"时才选 T_CUM 累计端点
